...
public static class Colors
{
    ...
    private const string OpaqueHex = "FF";

    private const string RgbaOpenParens = "rgba(";
    ...
    public static byte[] RgbaToArray(string rgba)
    {
        if ((!rgba.StartsWith("Rgba32(") || !rgba.StartsWith(RgbaOpenParens)) && !rgba.EndsWith(')') && !rgba.Contains(Comma))
        ...
        string[] rgbaStringArray = rgba.Replace("Rgba32", string.Empty)
            ...
            .Replace(")", string.Empty)
            .Split(Comma, StringSplitOptions.TrimEntries);
    ...
    public static string SystemHexToWebHex(string systemHex)
    {
        if (IsValidHex(in systemHex))
            return string.Concat(systemHex.ToUpper()[2..], systemHex.ToUpper()[..2]);

        throw HexStringError;
    }

    public static string WebHexToRgba(string webHex)
    {
        if (webHex.Length == 6)
            webHex = string.Concat(webHex.ToUpper(), OpaqueHex);

        if (IsValidHex(in webHex))
        {
            var rgba = webHex.ToUpper();
            return string.Concat(RgbaOpenParens, ConvertDoubleHexToDigit(rgba[..2]), Comma,
                ConvertDoubleHexToDigit(rgba.Substring(2, 2)), Comma,
                ConvertDoubleHexToDigit(rgba.Substring(4, 2)), Comma,
                ToAlphaValue(rgba[6..]),
                ')');
        }
    ...
    public static string WebHexToSystemHex(string webHex)
    {
        if (webHex.Length == 6)
            webHex = string.Concat(webHex.ToUpper(), OpaqueHex);

        if (IsValidHex(in webHex))
            return string.Concat(webHex.ToUpper()[6..], webHex.ToUpper()[..6]);
